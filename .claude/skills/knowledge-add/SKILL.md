...
3. **雛形をコピーする。** `templates/<type>.md` を `knowledge/<kebab-case>.md` にコピーする。ファイル名は ASCII kebab-case の名詞句。
...
4. **frontmatter を埋める。**
   ...
   - `status` は `stable` (書いた時点で現役)。確かめた度合いは status ではなく type が持つ
...
`note` は「まだ確かめていない」印であって置き場所ではない。.claude/rules/knowledge-authoring.md「note を昇格させる」の 3 項目が揃ったら type を変え、`pnpm check` を通す。ファイルは動かさないので ID は変わらない。
