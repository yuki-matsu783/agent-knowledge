---
type: pattern
title: 通知しなかった判定も skip として記録し記録の欠如を縮退と読めるようにする
description: >-
...
- [Agent ツール周りの hook 入出力の一覧](agent-tool-hook-fields-reference.md)。記録を引く識別子 (`tool_response.agentId`) が入力側の `agent_id` と別名で、取り違えると常に縮退になる
- [サブエージェントは既定で background で走り PostToolUse Agent は起動直後に発火する](subagent-runs-in-background-by-default.md)。保険側の経路が起動直後に来る話
