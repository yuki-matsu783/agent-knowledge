...
description: >-
  ...
  repository root (after a cd, in a subdirectory launch, in a worktree), and why that is worse than
...
Claude が `cd` した後、あるいはサブディレクトリや worktree で起動したセッションでは、この相対パスが解決できず `bash` が終了 127 で落ちる。
