...
title: ガード hook は enable / dry-run / disable の 3 モードで運用すべき
description: >-
  ...
  Use when adding a PreToolUse deny whose pattern might misfire, when a guard needs to be disabled
...
keywords: [hook, dry-run, ドライラン, 無効化, 環境変数, GUARD_MODE, enable, disable, ログ, JSON Lines, logs, 誤爆, 空振り, 段階導入, PreToolUse, exit 2, systemMessage, disableAllHooks, settings.json, env]
status: stable
...
# ガード hook は enable / dry-run / disable の 3 モードで運用すべき
...
| モード | 判定 | 出口 | 用途 |
|---|---|---|---|
| `enable` (既定) | する | HIT なら exit 2 | 通常運用 |
| `dryrun` | する | HIT でも exit 0。`additionalContext` で助言だけ返す | 新しいルールの慣らし |
| `disable` | しない | 即 exit 0 | この 1 本だけ切りたいとき |

既定を enable にする。変数が無い状態が一番強い側になるようにして、設定漏れがガードの消失にならないようにする。

```sh
#!/bin/sh
# PreToolUse (Write|Edit): 生成物の手編集を止める。
# GUARD_MODE=enable (既定) | dryrun (止めずに記録と助言) | disable (判定しない)
mode=${GUARD_MODE:-enable}
log() {
  mkdir -p logs
  printf '{"ts":"%s","hook":"protect-generated","mode":"%s","result":"%s","target":"%s"}\n' \
    "$(date -u +%Y-%m-%dT%H:%M:%SZ)" "$mode" "$1" "$2" >> logs/hooks.jsonl
}
if [ "$mode" = disable ]; then log disable ""; exit 0; fi
...
if [ "$mode" = dryrun ]; then
  printf '{"hookSpecificOutput":{"hookEventName":"PreToolUse","additionalContext":"[dry-run] enable なら止めていた (%s): %s"}}\n' "$hit" "$f"
  exit 0
fi
...
1. **判定はモードの外に 1 つだけ置く。** モードが変えるのは判定結果の扱いだけにする。dryrun と enable で判定コードが分かれると、dry-run で得た結果が本番の保証にならない
...
新しいガードは dryrun で入れる。数日運用してログの `result` を数え、HIT が想定どおりで pass に取りこぼしが無いことを確かめてから enable に上げる。

## 適用条件

効くのは、パスやコマンド文字列の一致で判定するガードで、誤爆の可能性があるもの。判定が純粋な関数で、dry-run しても副作用が出ないことが前提になる。

**disable を渡す経路を限定できることも前提。** hook は Claude Code のプロセスの環境を継承する。Bash ツールの中で `export` しても、それは別のプロセスなので hook には届かない。
経路は実質 2 つで、性質がまったく違う。

- **claude を起動したシェルの環境**。人が `GUARD_MODE=disable claude` と打つときだけ入る。セッション中のエージェントからは触れない。disable はここからだけ渡す
- **settings.json の `env` ブロック**。ここに書くとエージェントが編集でき、live reload で効いてしまう。disable の値をここに書かない

全部の hook を止めてよいなら、自前の disable モードより公式の `disableAllHooks` を使う。自前の disable モードを作る理由は、公式には**個別の hook だけを止める手段が無い**ことにある。

注入系の hook にはモードは要らない。落ちても素通りが既定なので、dry-run と disable は最初から実質そうなっている
...
- **disable はガードを外す正規の経路を作ることになる**。守りたい線が敵対的な回避にも耐える必要があるなら、disable を実装せず、管理者権限側 (managed settings、OS のファイル権限) に寄せる
- ログは放っておくと増える。日数か行数で切る処理を入れる。判定のたびに追記するので、ガードが毎ツール呼び出しで走るなら行数は速く伸びる
- モードの値は hook ごとに分けるか揃えるかを決める必要がある。揃えると 1 つの変数で全ガードが disable になり、disable の危険度が上がる

## 関連

- [ガード hook の 1 回の判定](../../diagrams/guard-hook-evaluation.lifecycle.html)。enable / dryrun / disable が出口を選ぶ段に当たることを示した archify の状態遷移図
...
- [ガードの設定と hook スクリプト自身をエージェントから守る](../20-PreToolUse/protect-guard-config-from-the-agent.md) — disable の経路を settings.json に置いてはいけない理由
