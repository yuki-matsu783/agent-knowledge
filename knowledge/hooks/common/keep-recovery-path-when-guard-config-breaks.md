...
同じ構造の穴が 3 つ見つかった。相対パス登録による全 deny、設定破損時の拒否、`jq --slurpfile` の副入力破損。どれも「ガード hook が自分を止めて回復手段を奪う」形をしている。
...
3. **読むだけの操作と、提供コマンドによる状態遷移は宣言に依らず常に許可する。** 宣言漏れで「ファイルを読む」「完了コマンドを打つ」が止まると、ヘッドレスでは宣言漏れ 1 つでセッションが終わる。
