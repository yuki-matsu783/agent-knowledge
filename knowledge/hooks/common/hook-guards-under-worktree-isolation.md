...
- `cd` による迂回面は `CLAUDE_BASH_MAINTAIN_PROJECT_WORKING_DIR` で狭められる。公式 env-vars ページによると、これを設定するとメインセッションの Bash / PowerShell コマンドのたびに
  作業ディレクトリが起動時の場所へ戻る (既定では前のコマンドの `cd` が次にも残る)。「`cwd` から上向きに `.claude` を探す」ガードが、前のターンの `cd` の影響を受けなくなる。
  1 コマンドの中の `cd x && ...` は防げないので、コマンド内の `cd` は従来どおり判定側で見る
