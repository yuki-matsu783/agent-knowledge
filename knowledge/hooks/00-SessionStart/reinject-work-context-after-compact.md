...
title: compact 後は SessionStart hook で作業コンテキストを再注入すべき
description: >-
...
([Windows では hook の "bash" が WSL のスタブに解決されて無言で動かない](../common/bash-hook-resolves-to-wsl-stub-on-windows.md))。
...
- [タスクの切れ目で /compact と /clear をユーザに依頼させる](../22-PostToolUse/ask-user-to-reset-context-at-task-boundaries.md)。境界で compact させる側。こちらは compact 後の回復側
- [Gemini CLI には圧縮後に発火する hook が無い](../01-PreCompact/gemini-cli-no-post-compress-hook.md)。同じことを Gemini CLI でやろうとしたときの制約
