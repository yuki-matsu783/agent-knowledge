...
[同じイベントの hook は並列に走り](../common/hooks-run-in-parallel-not-in-array-order.md)、PostToolUse は並列ツール呼び出しでも同時に発火するので、競合は 3 種類ある。
...
- [同じイベントの hook は並列に走り settings.json の配列順は実行順ではない](../common/hooks-run-in-parallel-not-in-array-order.md)。競合の出所
- [タイムアウトした hook はガードにならず素通りする](../common/hook-timeout-fails-open.md)。`trap` が効かない打ち切りの話
