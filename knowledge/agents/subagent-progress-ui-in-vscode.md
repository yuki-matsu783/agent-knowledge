...
- **サブエージェントの内部は親の JSONL には出ない (2026-09-05 に実測)。** 子のツール呼び出しは
  `<session-id>/subagents/agent-<agentId>.jsonl` に 1 件ずつ残り、親には `Agent` の tool_use と tool_result しか無い
  ([サブエージェントの transcript は別ファイルにツール呼び出しごと残る](../workflow/subagent-transcript-is-separate-file-with-every-tool-call.md))。
  つまり親を tail するだけでは足りず、`subagents/` の出現と各ファイルの最終追記時刻を監視する必要がある。
  取れる粒度は十分なので、「詰まっている」の検出は設計どおりに作れる
