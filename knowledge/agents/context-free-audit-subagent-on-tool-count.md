...
sources:
  ...
  - knowledge/hooks/common/hook-timeout-fails-open.md
...
[hook はタイムアウトすると素通りする](../hooks/common/hook-timeout-fails-open.md) の回避策 1 と 6 にそのまま沿う。
...
- **判定ファイルをいつ消すか。** 止めた時点で消すと、本体が理由を読んでユーザーに確認して続行できる。
  ...
  ([ガードの設定と hook 自身をエージェントから守る](../hooks/20-PreToolUse/protect-guard-config-from-the-agent.md) と同じ問題)
