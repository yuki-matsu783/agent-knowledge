...
その 7 件を直すために書いた決定 (`curl` をガード hook が止める分類に加える) 自体が、また同じ型を作った。共通仕様と guard の仕様は更新したのに、
調査タスクの要件書には「この禁止はガード hook では止めず自制による」という逆の記述が残り、ライブラリの分類表には新しい分類が無いままだった。
...
- [決定記録は背景・決定・理由・却下した案・影響の 5 節に固定するとよさそう](decision-record-fixed-five-sections.md)。「影響」節を含む節構成
